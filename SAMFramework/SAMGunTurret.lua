behaviour("SAMGunTurret") --v1.0.0

function SAMGunTurret:Start()
...
function SAMGunTurret:LateUpdate()
    ...
    if not self.vehicle.playerIsInside or self.radar.lockedVehicle == nil or not self.autoAim then
        ...
        local tof = Vector3.Distance(pos, self.pitch.position) / self.bulletVelocity
    ...
    self.pitch.rotation = Quaternion.RotateTowards(self.pitch.rotation, pitchTarget, self.speed * Time.deltaTime)
    -- self.lastRot = self.pitch.rotation
